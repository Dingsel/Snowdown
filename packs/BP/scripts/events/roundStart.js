...
function headCheck(dimension, x, y, z) {
    return new Promise((resolve) => {
        const id = system.runInterval(() => {
            ...
            const block = dimension.getBlock({ x: x + randInt1, y, z: z + randInt2 })
...
core.onRoundStart(() => {
    ...
    spawnLoop(rating).catch(e => console.error(e, e.stack))
})
...
async function spawnLoop(mobRating) {
    ...
    const loc = { x: Number(coords[0]), y: 2, z: Number(coords[1]) }
    const spawnedEntity = ow.spawnEntity(entity, loc)
    //world.sendMessage(`spawned entity at ${spawnedEntity.location.x} ${spawnedEntity.location.y} ${spawnedEntity.location.z}`)
...
async function endRound() {
    ...
    const entities = ow.getEntities({ families: ["enemy"] })
    const boss = ow.getEntities({ families: ["boss"] })
    if (boss?.length >= 1) {
    ...
    if (world.getDynamicProperty("roundActive") && 5 >= entities?.length) {
        ...
        ow.spawnEntity("dest:present", { x: 0, y: 2, z: -6 })
        for (const player of world.getAllPlayers()) {
            world.playSound("random.levelup", player.location, { pitch: 1.5 })
...
system.runInterval(() => {
    for (const player of world.getPlayers()) {
        player.addEffect("saturation", 999999, { showParticles: false })
    }
}, 100)
